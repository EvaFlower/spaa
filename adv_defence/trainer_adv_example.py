...
class TrainerAdvExample(object):
    ...
    def train(self):
        ...
        for step in trange(self.start_step, self.config.max_step, ncols=80):
            ...
            # attr_label = self._get_variable(data[2].type(torch.FloatTensor))
            ...
            for g_iter_step_no in range(self.config.train_g_iter):
                ...
                adv_img_for_g, attr_pred, mu, logvar = self.NoiseGenerator.forward_attr_disentalgle(img_grad_advimg,
                                                                                                    double_label, g_z)
                attr_adv_img_for_g, attr_img_for_g, attr_pred, mu, logvar = self.AttrAdvNoiseGenerator.forward()




                # clamping it once again to image boundary
                adv_img_for_g = torch.clamp(adv_img_for_g, 0.0, 1.0)
                
                adv_noise = torch.sub(adv_img_for_g, double_real_img)  # TODO: real image -> generated attr-perturb img
                ...
                # Add MSP-loss for reconstruction, MSP-loss for matrix-subspace projection, VAE-loss.
                ...
                if self.config.g_attribute_disentangle and ((self.config.g_attribute_disentangle_style == 1) or
                    ...
                    # TODO: change attr_pred -> attr_label
                    # loss, l_rec, l_vae, l_msp = msp_loss(adv_img_for_g, double_real_img,
                    #                                                      attr_label, attr_pred, mu, logvar)

                    # Warning: When don't have labels attributes, use attr_pred as attr_label. avoid attribute
                    # disentanglement.
                    msp_loss_, l_rec, l_vae, l_msp = msp_loss(adv_img_for_g, double_real_img,
                                                                         attr_pred, attr_pred, mu, logvar)
                    msp_loss_sum += msp_loss_



                # Diversity loss: compute & accumulate DSGAN gradient
                # TODO: the diversity of noise can not be used
                #  1. *DONE* Compute noise via torch.sub(real_img, adv_img).detach()
                #  2. Remove diversity loss first by setting g_z_dim=0. Deal with it later.
                if (self.config.g_z_dim > 0) and \
                        ((self.config.g_mini_update_style >= 2) or (g_iter_step_no + 1 == self.config.train_g_iter)):

                    # compute the dsgan loss and add it to g_loss
                    # TODO: Notice that the clamp_noise here is the diff between [real_img, adv_img],
                    #  rather than [attr_pert_img, attr_adv_pert_img].
                    #  Should go back to modify clamp_noise.
                ...
                if g_iter_step_no + 1 != self.config.train_g_iter:
                    ...
                    if double_adv_img is not None:
                        double_adv_img = adv_img_for_g

                update_list.append(adv_img_for_g.detach())

            g_loss_sum = proxy_loss_sum + dsgan_loss_sum + msp_loss_sum  # adversarial loss + diversity loss + msp loss
            g_loss_sum.backward()
            ...
            if self.config.train_g_iter > 0:
                ...
                self.tensorboard.add_scalar('train/msp_loss_sum', msp_loss_sum.data, step)
                self.tensorboard.add_scalar('train/loss_rec', l_rec, step)
                self.tensorboard.add_scalar('train/loss_vae', l_vae, step)
                self.tensorboard.add_scalar('train/loss_msp', l_msp, step)
