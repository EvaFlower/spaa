...
def _loss_msp(M, attr_label, attr_pred):
    """
    ...
    :return:  MSP loss
    """
    L1 = F.mse_loss((attr_pred @ M.t()).view(-1), attr_label.view(-1), reduction="none").sum()
    L2 = F.mse_loss((attr_label @ M).view(-1), attr_pred.view(-1), reduction="none").sum()
    return L1 + L2


def loss(pred, real_img, attr_label, attr_pred, mu, logvar):
    L_rec = _loss_reconstruction(pred, real_img)
    L_vae = _loss_vae(mu, logvar)
    L_msp = _loss_msp(attr_label, attr_pred)
